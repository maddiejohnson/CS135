...
(define (CRT-exists? cong-sys)
  (cond [(null? (cdr cong-sys)) #t]
        [(existsHelper (map (lambda (e) (euclid-gcd (cadar cong-sys) e))
                (cdr (map cadr cong-sys)))) (CRT-exists? (cdr cong-sys))]
        [else #f]))
        

(define (existsHelper lst)
  (cond [(and (null? (cdr lst)) (equal? (car lst) 1)) #t]
        [(not (equal? (car lst) 1)) #f]
        [else (existsHelper (cdr lst))]))
...
(define (CRT-helper cong-sys m)
  (if (null? (cdr cong-sys)) (*(caar cong-sys)(/ m (cadar cong-sys))(mul-inv (/ m (cadar cong-sys))(cadar cong-sys)))
   (+(*(caar cong-sys)(/ m (cadar cong-sys))(mul-inv (/ m (cadar cong-sys))(cadar cong-sys)))
...
(define (CRT cong-sys)
  (if (CRT-exists? cong-sys) (modulo (CRT-helper cong-sys (m cong-sys)) (m cong-sys))
      -1))