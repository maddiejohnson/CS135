...
(define (indegree Es v)
   (cond [(null? Es) 0]
        [(or (equal? v (car Es))(equal? v (cadar Es)))
         (+ 1 (indegree (cdr Es) v))]
...
(define (find-top G)
  (topHelper (get-vertices G) (get-edges G)))
  
(define (topHelper vertices edges)
  (cond [(null? vertices) (tsort-error)]
        [(zero? (indegree edges (car vertices))) (car vertices)]
        [else (topHelper (cdr vertices) edges)]))
