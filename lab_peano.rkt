...
(define (pred p)
  (if (pz? p) (eopl:error "pred error: zero has no predecessor!")
      (car p)))
...
(define (p= a b)
  (cond [(or (and(pz? a)(not(pz? b))) (and(pz? b)(not(pz? a)))) #f]
        [(and (pz? a)(pz? b)) #t]
        [else (p= (pred a) (pred b))]))
...
;; Type signature: (p> peano peano) -> bool
(define (p> a b)
  (cond [(and (pz? b)(not(pz? a))) #t]
        [(p= a b) #f]
        [(not(or (pz? b)(pz? a))) (p> (pred a) (pred b))]
        [else #f]))
...
(define (helpPrime d p)
  (cond [(or (pz? p)(pz? (pred p))) #f]
        [(pz? (pred d)) #t]
        [(pz? (p% p d)) #f]
        [else (helpPrime (pred d) p)]))
