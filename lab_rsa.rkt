...
(define (mod-exp b e m)
  (cond [(equal? e 0) 1]
        [(equal? (modulo e 2) 0) (modulo (* (mod-exp b (/ e 2) m)(mod-exp b (/ e 2) m)) m)]
        [else (modulo (modulo (* (modulo b m) (mod-exp b (- e 1) m)) m) m)]))
...
(define (encrypt plaintext key)
  (map (lambda (n) (mod-exp n (cadr key) (car key))) (map char->integer (string->list plaintext))))
...
(define (decrypt ciphertext key)
   (list->string(map integer->char (map (lambda (n) (mod-exp n (cadr key) (car key))) ciphertext))))
