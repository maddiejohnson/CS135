...
(define (inverse R)
  (if (null? R) '() (cons (reverse(car R)) (inverse(cdr R)))))
...
(define (relates-to v R)
  (cond [(null? R) '()]
        [(equal? v (caar R)) (cons (car(cdr(car R))) (relates-to v (cdr R)))]
        [(equal? v (car(cdr(car R)))) (cons (car R) (relates-to v (cdr R)))]
