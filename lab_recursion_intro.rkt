...
(define (sum lst)
  (define total 0)
  (cond ((null? lst) 0)
  ((null? (cdr lst)) (+ total (car lst)))
  (else (+ total (car lst) (sum(cdr lst))))))
...
(define (product lst)
  ...
  (cond ((null? lst) 1)
  ((null? (cdr lst)) (* total (car lst)))
  (else (* total (car lst) (product (cdr lst))))))
...
(define (map f lst)
  ...
  (cond ((null? lst) result)
        ((null? (cdr lst)) (cons (f (car lst)) result))
...
(define (filter pred lst)
  (define result (list))
  (cond ((null? lst) result)
        ((null? (cdr lst)) (if (pred(car lst)) (cons (car lst) result) result))
