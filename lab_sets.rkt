...
(define (intersection LA LB)
  (if (null? LA)'()
  (if (element? (car (make-set LA)) (make-set LB))
      (cons (car (make-set LA)) (intersection (cdr (make-set LA)) (make-set LB)))
      (intersection (cdr (make-set LA)) (make-set LB)))))
...
(define (set-equal? SA SB)
  (cond [(null? SA) #t]
        [(element? (car SA) SB) (set-equal? (cdr SA) SB)]
        [else #f]))
...
(define (set-difference LA LB)
  (cond [(null? LA) '()]
        ...
        [else (make-set (cons (car LA) (make-set (set-difference (cdr LA) LB))))]))
...
(define (remove e S)
  (if (null? S)'()
  (if (equal? e (car S)) (remove e (cdr S)) (cons (car S) (remove e (cdr S))))))
