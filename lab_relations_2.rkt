...
(define (compose RO RI)
  (cond [(null? RI) '()]
        [(null? RO) '()]
        [(equal? (car(cdr(car RI))) (car(car RO)))
         (make-set (append (append (cons (list (car(car RI))(car(cdr(car RO))))(compose (cdr RO)(cdr RI)))(compose (cdr RO) RI))(compose RO (cdr RI))))]
        [else (make-set (append (append (compose (cdr RO) (cdr RI))(compose (cdr RO) RI))(compose RO (cdr RI))))]))
...
(define (powerHelper R k previous)
  (if (= k 0)
      '()
      (if (= k 1)
          previous 
      (powerHelper R (- k 1) (compose R previous)))))
...
(define (transitiveHelp relation k storage)
  (if (subset? (power relation k) storage)
      storage (transitiveHelp relation (+ k 1) (union (power relation k) storage))))
                      
(define (transitive-closure relation)
  (transitiveHelp relation 1 '()))
...
(define (EQ-relation? R n)
  (and (symmetric? R)(transitive? R)(reflexive? R n)))
