...
(define (get-vertices G)
  (usort(cond [(null? G) '()]
        [(null? (cdr G))(cons(caar G)(cdar G))]
        [(pair? (car G))(append(list(caar G)(cadar G))(get-vertices (cdr G)))]
        [else (cons (caar G)(get-vertices (cdr G)))])))
...
(define (count lst v)
    (cond ((null? lst) 0)
          ((= (car lst) v) (+ 1 (count (cdr lst) v)))
          (else (count (cdr lst) v))))

(define (unlist G)
  (cond [(null? G) '()]
        [(null? (cdr G))(cons(caar G)(cdar G))]
        [(pair? (car G))(append(list(caar G)(cadar G))(unlist (cdr G)))]
        [else (cons (caar G)(unlist (cdr G)))]))
...
(define (all-even? G)
  (even-helper G (get-vertices G)))


(define (even-helper lst sorted)
  (cond [(null? sorted) #t]
        [(even? (degree lst (car sorted))) (even-helper lst (cdr sorted))]
        [else #f]))
...
(define (adjacent G v)
  (usort
   (cond [(null? G) '()]
        [(equal? v (caar G))(cons(cadar G)(adjacent (cdr G) v))]
        [(equal? v (cadar G))(cons(caar G)(adjacent (cdr G) v))]
...
(define (connected-helper lst sorted)
  (cond [(null? sorted) #t]
        [(equal? (usort(dfs lst '() (car sorted)))(get-vertices lst))(connected-helper lst (cdr sorted))]
        [else #f]))
